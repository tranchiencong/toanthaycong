...
export function SubjectManagerClient({ subjects }: { subjects: SubjectWithCount[] }) {
  ...
  const handleStartEdit = (subj: SubjectWithCount) => {
  ...
  const handleSaveEdit = (id: string) => {
    ...
    startTransition(async () => {
      const res = await updateSubjectAction(id, formData)
      if (res.success) {
  ...
  const handleDelete = (subj: SubjectWithCount) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa môn học "${subj.name}"?`)) return

    startTransition(async () => {
      const res = await deleteSubjectAction(subj.id)
      if (res.success) {
    ...
    <div className="bg-white border border-slate-200 shadow-2xs">
      <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-slate-900">Danh Sách Môn Học / Phân Môn</h2>
      ...
      {isAdding && (
        <form onSubmit={handleAddSubmit} className="p-4 bg-blue-50/40 border-b border-blue-100 space-y-3">
          <p className="text-xs font-bold text-blue-950 uppercase tracking-wider">
            Thêm môn học mới (Ví dụ: Đại số, Hình học, Xác suất, Toán Tư Duy...)
          </p>
          <div>
            <label className="block text-[11px] font-semibold text-slate-700 mb-1">Tên môn học *</label>
            <input
              type="text"
              value={newName}
              onChange={(e) => setNewName(e.target.value)}
              placeholder="Ví dụ: Đại Số 12 hoặc Toán Tư Duy"
              required
              className="w-full text-xs px-3 py-2 border border-slate-300 focus:outline-none focus:border-blue-900 bg-white max-w-md"
            />
          </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-50/80 text-slate-600 font-semibold border-b border-slate-200">
              <th className="py-3 px-4">Tên Môn Học / Phân Môn</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {subjects.map((subj) => {
              ...
              if (isEditing) {
                return (
                  <tr key={subj.id} className="bg-amber-50/40">
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="inline-flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        disabled={isPending || subj._count.courses > 0}
                        title={subj._count.courses > 0 ? "Không thể xóa vì đang có khóa học" : "Xóa"}
